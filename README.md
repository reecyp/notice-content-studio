...
npm run check    # typecheck + uid + fonts + layout grammar + ledger assertions
...
| `docs/database.md` | the send ledger: what it stores, why uid, and what a batch send still needs |
...
| `scripts/schema.sql` | the two tables, `video` and `send` |
...
disk cannot know — that a video went to TikTok, when, and what came back. It is optional, and the
studio runs unchanged without it. `docs/database.md` covers the split and the two tables.
